...
class AddCategoryIdToRestaurantsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('restaurants', function (Blueprint $table) {
            $table->integer('category_id')              // category_idを追加
                 ...
                 ->unsigned()                            // 正負の符号無し属性を設定   integerの時に+-を無くす
                 ->default(1);                           // 空で外部キーを設定できないのでデフォルトの制約を追加
             $table->foreign('category_id')              // category_idに外部キーを設定する
                 ->references('id')->on('categories')    // categoriesテーブルのidカラムを外部キーにする 紐付け
    ...
    {
        Schema::table('restaurants', function (Blueprint $table) {
            $table->dropForeign('restaurants_category_id_foreign'); //外部キー制約の削除 接続解除
